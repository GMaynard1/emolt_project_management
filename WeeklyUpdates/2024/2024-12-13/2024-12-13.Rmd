...
Several of the original eMOLT participants supplied lobster haul data in addition to temperature data. Therese Sauvageau out of Beverly Mass, for example, still submits here records each year so that we MIGHT someday be able to determine the correlation (if any) between catch and temperature.  As seen in the figure below, she also recorded the surface temperature as measured by her hull-mounted thermistor. While we do not expect these thermistors to be highly accurate, they do provide a means to an approximate stratification index (ie difference bewteen surface and bottom temperatures). You can see the raw bottom temperature data is filtered to remove those times when the trap was evidently hauled (red spikes).  As usual, it is difficult to detect any obvious catch vs temperature relationships, we can see that it may be a function of the stratification and/or variability rather than the absolute value of temperature. 
![TS02 temp & catch](mts0213.png)
