...
## Develop a linear regression
dop_lm=lm(data$forecast_temp~data$avg_temp)
pred = dop_lm$coefficients[1]+dop_lm$coefficients[2]*seq(-1,45,1)
plot(
...
text(
  x=7,
  y=30,
  label = round(summary(dop_lm)$r.squared,3)
)
text(
  x=5,
  y=27,
  label = paste0("RMSE = ",round(mean(dop_lm$residuals^2),3))
)
text(
  x=5,
  y=24,
...
## Develop a linear regression
nec_lm=lm(data$forecast_temp~data$avg_temp)
pred = nec_lm$coefficients[1]+nec_lm$coefficients[2]*seq(-1,45,1)
plot(
  data$forecast_temp~data$avg_temp,
  xlim=c(0,35),
  ylim=c(0,35),
  ...
  col=data$scalecolor,
  pch=16,
  main="Predicted vs. Observed Bottom Temperatures (GOM7)"
)
points(
  data$forecast_temp~data$avg_temp,
  pch=1,
...
text(
  x=7,
  y=30,
  label = round(summary(nec_lm)$r.squared,3)
)
text(
  x=5,
  y=27,
  label = paste0("RMSE = ",round(mean(nec_lm$residuals^2),3))
)
text(
  x=5,
  y=24,
...
plot(
  ...
  main="GOM7 Predicted - eMOLT Obs (deg F)",
  sub=paste0(
    lubridate::floor_date(min(lubridate::ymd(data$date)),"day"),
    " to ",
    lubridate::ceiling_date(max(lubridate::ymd(data$date)),"day")
